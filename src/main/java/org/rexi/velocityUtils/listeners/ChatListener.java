...
public class ChatListener {
    ...
    public ChatListener(ConfigManager configManager, ProxyServer server,
    ...
    public void onChat(PlayerChatEvent event) {
        ...
        boolean inStaffChat = StaffChatCommand.isInStaffChat(player.getUniqueId());
        boolean inAdminChat = AdminChatCommand.isInAdminChat(player.getUniqueId());

        if (!inStaffChat && !inAdminChat) return;

        event.setResult(PlayerChatEvent.ChatResult.denied()); // Cancela el mensaje normal
        ...
        if (inStaffChat) {
            ...
            Component chatMessage = LegacyComponentSerializer.legacyAmpersand().deserialize(format);
